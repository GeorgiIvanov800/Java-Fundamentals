...
public class HouseParty {
    public static void main(String[] args) {
        ...
        for (int count = 1; count <= countCommands; count++) {
            ...
            //•   "{name} is going!" -> split -> ["{name}", "is", "going!"] -> collect -> {"{name}", "is", "going!"} -> 3 elements
            //•   "{name} is not going!" -> split -> ["{name}", "is", "not", "going!"] -> collect -> {"{name}", "is", "not", "going!"} -> 4 elements
            List<String> commandParts = Arrays.stream(command.split(" ")).collect(Collectors.toList());
            String name = commandParts.get(0);
            //is going -> it's there or it's not
            ...
            //is not going -> there is (remove) or there is not
